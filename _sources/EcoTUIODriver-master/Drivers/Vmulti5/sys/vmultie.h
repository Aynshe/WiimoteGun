...
#define DRIVERNAME                 "vmultie.sys: "

#define vmultie_POOL_TAG            (ULONG) 'luMV'
#define vmultie_HARDWARE_IDS        L"ecologylab\\vmultie\0\0"
#define vmultie_HARDWARE_IDS_LENGTH sizeof(vmultie_HARDWARE_IDS)

#define NTDEVICE_NAME_STRING       L"\\Device\\vmultie"
#define SYMBOLIC_NAME_STRING       L"\\DosDevices\\vmultie"
...
typedef struct _vmultie_CONTEXT 
{
...
} vmultie_CONTEXT, *Pvmultie_CONTEXT;

WDF_DECLARE_CONTEXT_TYPE_WITH_NAME(vmultie_CONTEXT, vmultieGetDeviceContext)
...
EVT_WDF_DRIVER_UNLOAD vmultieDriverUnload;

EVT_WDF_DRIVER_DEVICE_ADD vmultieEvtDeviceAdd;

EVT_WDFDEVICE_WDM_IRP_PREPROCESS vmultieEvtWdmPreprocessMnQueryId;

EVT_WDF_IO_QUEUE_IO_INTERNAL_DEVICE_CONTROL vmultieEvtInternalDeviceControl;
...
#if DBG
#define vmultiePrint(dbglevel, dbgcatagory, fmt, ...) {          \
    if (vmultieDebugLevel >= dbglevel &&                         \
        (vmultieDebugCatagories && dbgcatagory))                 \
    {                                                           \
        DbgPrint(DRIVERNAME);                                   \
...
#else
#define vmultiePrint(dbglevel, fmt, ...) {                       \
}
#endif

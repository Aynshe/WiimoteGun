...
echo %BASEDIR%\tools\devcon\i386\devcon.exe install vmultid.inf ecologylab\vmultid > bin\install_driver.bat

:: create bat file for easier testing
echo %BASEDIR%\tools\devcon\i386\devcon.exe remove ecologylab\vmultid > bin\test.bat
echo cd .. >> bin\test.bat
echo cmd /c buildme.bat >> bin\test.bat
echo cd bin >> bin\test.bat
echo cmd /c install_driver.bat >> bin\test.bat
echo echo After keypress, launching testvmultid >> bin\test.bat
echo pause >> bin\test.bat
echo testvmultid /joystick >> bin\test.bat

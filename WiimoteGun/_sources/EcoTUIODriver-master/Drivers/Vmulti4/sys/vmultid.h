...
#define DRIVERNAME                 "vmultid.sys: "

#define vmultid_POOL_TAG            (ULONG) 'luMV'
#define vmultid_HARDWARE_IDS        L"ecologylab\\vmultid\0\0"
#define vmultid_HARDWARE_IDS_LENGTH sizeof(vmultid_HARDWARE_IDS)

#define NTDEVICE_NAME_STRING       L"\\Device\\vmultid"
#define SYMBOLIC_NAME_STRING       L"\\DosDevices\\vmultid"
...
typedef struct _vmultid_CONTEXT 
{
...
} vmultid_CONTEXT, *Pvmultid_CONTEXT;

WDF_DECLARE_CONTEXT_TYPE_WITH_NAME(vmultid_CONTEXT, vmultidGetDeviceContext)
...
EVT_WDF_DRIVER_UNLOAD vmultidDriverUnload;

EVT_WDF_DRIVER_DEVICE_ADD vmultidEvtDeviceAdd;

EVT_WDFDEVICE_WDM_IRP_PREPROCESS vmultidEvtWdmPreprocessMnQueryId;

EVT_WDF_IO_QUEUE_IO_INTERNAL_DEVICE_CONTROL vmultidEvtInternalDeviceControl;
...
#if DBG
#define vmultidPrint(dbglevel, dbgcatagory, fmt, ...) {          \
    if (vmultidDebugLevel >= dbglevel &&                         \
        (vmultidDebugCatagories && dbgcatagory))                 \
    {                                                           \
        DbgPrint(DRIVERNAME);                                   \
...
#else
#define vmultidPrint(dbglevel, fmt, ...) {                       \
}
#endif

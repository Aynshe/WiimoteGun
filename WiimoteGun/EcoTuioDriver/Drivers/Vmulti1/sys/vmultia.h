...
#define DRIVERNAME                 "vmultia.sys: "

#define vmultia_POOL_TAG            (ULONG) 'luMV'
#define vmultia_HARDWARE_IDS        L"ecologylab\\vmultia\0\0"
#define vmultia_HARDWARE_IDS_LENGTH sizeof(vmultia_HARDWARE_IDS)

#define NTDEVICE_NAME_STRING       L"\\Device\\vmultia"
#define SYMBOLIC_NAME_STRING       L"\\DosDevices\\vmultia"
...
typedef struct _vmultia_CONTEXT 
{
...
} vmultia_CONTEXT, *Pvmultia_CONTEXT;

WDF_DECLARE_CONTEXT_TYPE_WITH_NAME(vmultia_CONTEXT, vmultiaGetDeviceContext)
...
EVT_WDF_DRIVER_UNLOAD vmultiaDriverUnload;

EVT_WDF_DRIVER_DEVICE_ADD vmultiaEvtDeviceAdd;

EVT_WDFDEVICE_WDM_IRP_PREPROCESS vmultiaEvtWdmPreprocessMnQueryId;

EVT_WDF_IO_QUEUE_IO_INTERNAL_DEVICE_CONTROL vmultiaEvtInternalDeviceControl;
...
#if DBG
#define vmultiaPrint(dbglevel, dbgcatagory, fmt, ...) {          \
    if (vmultiaDebugLevel >= dbglevel &&                         \
        (vmultiaDebugCatagories && dbgcatagory))                 \
    {                                                           \
        DbgPrint(DRIVERNAME);                                   \
...
#else
#define vmultiaPrint(dbglevel, fmt, ...) {                       \
}
#endif

...
#define DRIVERNAME                 "vmultib.sys: "

#define vmultib_POOL_TAG            (ULONG) 'luMV'
#define vmultib_HARDWARE_IDS        L"ecologylab\\vmultib\0\0"
#define vmultib_HARDWARE_IDS_LENGTH sizeof(vmultib_HARDWARE_IDS)

#define NTDEVICE_NAME_STRING       L"\\Device\\vmultib"
#define SYMBOLIC_NAME_STRING       L"\\DosDevices\\vmultib"
...
typedef struct _vmultib_CONTEXT 
{
...
} vmultib_CONTEXT, *Pvmultib_CONTEXT;

WDF_DECLARE_CONTEXT_TYPE_WITH_NAME(vmultib_CONTEXT, vmultibGetDeviceContext)
...
EVT_WDF_DRIVER_UNLOAD vmultibDriverUnload;

EVT_WDF_DRIVER_DEVICE_ADD vmultibEvtDeviceAdd;

EVT_WDFDEVICE_WDM_IRP_PREPROCESS vmultibEvtWdmPreprocessMnQueryId;

EVT_WDF_IO_QUEUE_IO_INTERNAL_DEVICE_CONTROL vmultibEvtInternalDeviceControl;
...
#if DBG
#define vmultibPrint(dbglevel, dbgcatagory, fmt, ...) {          \
    if (vmultibDebugLevel >= dbglevel &&                         \
        (vmultibDebugCatagories && dbgcatagory))                 \
    {                                                           \
        DbgPrint(DRIVERNAME);                                   \
...
#else
#define vmultibPrint(dbglevel, fmt, ...) {                       \
}
#endif
